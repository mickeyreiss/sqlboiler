...
func (p *PostgresDriver) TranslateColumnType(c db.Column) db.Column {
	if c.Nullable {
		...
		case "bit", "interval", "uuint", "bit varying", "character", "money", "character varying", "cidr", "inet", "macaddr", "text", "uuid", "xml":
			c.TypeName = "string"
		case `"char"`:
			c.TypeName = "types.Byte"
		case "json", "jsonb":
			c.TypeName = "types.JSON"
		case "bytea":
			c.TypeName = "[]byte"
		case "boolean":
			c.TypeName = "bool"
		case "date", "time", "timestamp without time zone", "timestamp with time zone":
			c.TypeName = "time.Time"
		case "ARRAY":
			c.TypeName = getArrayType(c)
			// Make DBType something like ARRAYinteger for parsing with randomize.Struct
			c.DBType = c.DBType + *c.ArrType
...
func getArrayType(c db.Column) string {
	switch *c.ArrType {
